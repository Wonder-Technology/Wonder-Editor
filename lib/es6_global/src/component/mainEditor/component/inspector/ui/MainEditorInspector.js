...
function _getAllShowComponentList(allShowComponentsConfig, allComponentList) {
  return Js_list.filter((function (param) {
                ...
                var name = param[0];
                return OperateArrayUtils$WonderEditor.hasItem(allShowComponentsConfig$1.filter((function (param) {
                                  return +(param[/* componentName */0] === name);
                                })));
              }), allComponentList);
}

function _buildComponentUIComponent(type_, gameObjectComponent, store, dispatch, componentArray) {
  switch (type_) {
    case "cameraController" : 
    case "material" : 
        return componentArray;
    case "transform" : 
        return OperateArrayUtils$WonderEditor.push(ReasonReact.element(/* Some */[DomHelper$WonderEditor.getRandomKey(/* () */0)], /* None */0, MainEditorTransform$WonderEditor.make(store, dispatch, gameObjectComponent, /* array */[])), componentArray);
    default:
