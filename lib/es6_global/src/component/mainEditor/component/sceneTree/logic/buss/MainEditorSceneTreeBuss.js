...
function getDragedSceneGraphData(targetUid, dragedUid, sceneGraphArrayData) {
  return Contract$WonderLog.ensureCheck((function (r) {
                return Contract$WonderLog.test(Log$WonderLog.buildAssertMessage("the draged scene graph data == scene data from engine", "not"), (function () {
                              Log$WonderLog.printJson(r);
                              return Contract$WonderLog.assertTrue(Caml_obj.caml_equal(getSceneGraphDataFromEngine(MainEditorStateView$WonderEditor.prepareState(/* () */0)), r));
                            }));
