...
function setTransformParentKeepOrder(parentGameObject, childGameObject, param) {
  ...
  return /* tuple */[
          param[0],
          MainEditorTransformOper$WonderEditor.setTransformParentKeepOrder(MainEditorGameObjectOper$WonderEditor.getTransformComponent(parentGameObject, engineState), MainEditorGameObjectOper$WonderEditor.getTransformComponent(childGameObject, engineState), engineState)
        ];
}
...
function getDragedSceneGraphData(targetUid, dragedUid, sceneGraphArrayData) {
  return Contract$WonderLog.ensureCheck((function (dragedSceneGraph) {
                return Contract$WonderLog.test(Log$WonderLog.buildAssertMessage("the draged scene graph data == scene data from engine", "not"), (function () {
                              return Contract$WonderLog.assertTrue(Caml_obj.caml_equal(getSceneGraphDataFromEngine(MainEditorStateView$WonderEditor.prepareState(/* () */0)), dragedSceneGraph));
                            }));
