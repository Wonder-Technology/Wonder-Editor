...
function _getUniqueMapByComponentName(state, uiComponentName) {
  var match = state[/* mapState */1][/* componentsMap */0];
  if (match !== undefined) {
    var match$1 = MutableHashMapService$WonderCommonlib.get(uiComponentName, Caml_option.valFromOption(match));
...
function _createArgumentArray(uiComponentName, state, prop) {
  ...
  switch (type_) {
    case "function" : 
        var match = MutableHashMapService$WonderCommonlib.get(value, _getUniqueMapByComponentName(state, uiComponentName));
        if (match !== undefined) {
