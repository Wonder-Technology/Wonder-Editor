...
function MakeSameDataResult(Item) {
  ...
  var map = function (handleDataFunc, twoTrackInput) {
...
function map$1(handleDataFunc, twoTrackInput) {
...
function MakeRelationResult(Item) {
  ...
  var handleSuccess = function (handleSuccessFunc, result) {
    if (result.tag) {
      return result;
    } else {
      return Curry._1(handleSuccessFunc, /* () */0);
    }
  };
...
function handleSuccess(handleSuccessFunc, result) {
  if (result.tag) {
    return result;
  } else {
    return Curry._1(handleSuccessFunc, /* () */0);
