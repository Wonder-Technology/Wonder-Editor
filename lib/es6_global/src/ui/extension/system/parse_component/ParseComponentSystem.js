...
function _getUniqueMapByComponentName(state, uiComponentName) {
  ...
  if (match) {
    ...
    if (match$1) {
      ...
      return Log$WonderLog.fatal(Log$WonderLog.buildFatalMessage("_getUniqueMapByComponentName", "can\'t find " + (String(uiComponentName) + " in appState->mapState"), "", "", "uiComponentName:" + (String(uiComponentName) + "")));
    }
  } else {
