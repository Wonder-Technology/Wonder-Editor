...
function storeCopiedEngineHistoryState(store, param, historyState) {
  ...
  return EngineHistoryService$WonderEditor.storeHasCopyState(maxStackSize, param[1], EditorHistoryService$WonderEditor.storeState(maxStackSize, editorState, UIHistoryService$WonderEditor.storeUIState(maxStackSize, store, historyState)));
}

function storeHistoryState(store, param, historyState) {
  var editorState = param[0];
  var maxStackSize = RedoUndoSettingEditorService$WonderEditor.unsafeGetMaxStackSize(editorState);
  return EngineHistoryService$WonderEditor.storeNoCopyState(maxStackSize, param[1], EditorHistoryService$WonderEditor.storeState(maxStackSize, editorState, UIHistoryService$WonderEditor.storeUIState(maxStackSize, store, historyState)));
}
...
function undoHistoryState(store, dispatchFunc, param) {
  ...
  var editorState = EditorHistoryService$WonderEditor.undo(AllStateData$WonderEditor.getHistoryState(/* () */0), param[0]);
  var engineState = EngineHistoryService$WonderEditor.undo(AllStateData$WonderEditor.getHistoryState(/* () */0), param[1]);
...
function redoHistoryState(store, dispatchFunc, param) {
  ...
  var editorState = EditorHistoryService$WonderEditor.redo(AllStateData$WonderEditor.getHistoryState(/* () */0), param[0]);
  var engineState = EngineHistoryService$WonderEditor.redo(AllStateData$WonderEditor.getHistoryState(/* () */0), param[1]);
  StateEditorService$WonderEditor.setState(editorState);
  StateEngineService$WonderEditor.setState(engineState);
  Curry._1(dispatchFunc, [
        AppStore$WonderEditor.UpdateAction,
