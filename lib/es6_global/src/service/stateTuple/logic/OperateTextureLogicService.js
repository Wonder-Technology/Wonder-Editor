...
import * as NodeMapAssetService$WonderEditor from "../../state/asset/NodeMapAssetService.js";
...
function setTextureNameToEngineAndNodeMap(nodeId, texture, newName) {
  ...
  var __x = AssetTreeNodeUtils$WonderEditor.renameNodeResult(newName, SparseMapService$WonderCommonlib.unsafeGet(nodeId, NodeMapAssetService$WonderEditor.unsafeGetNodeMap(assetState)));
  ...
  return Contract$WonderLog.ensureCheck((function () {
                return Contract$WonderLog.test(Log$WonderLog.buildAssertMessage("the texture in nodeMap name should == engine name", "not"), (function () {
                              var param = SparseMapService$WonderCommonlib.unsafeGet(nodeId, NodeMapAssetService$WonderEditor.unsafeGetNodeMap(StateAssetService$WonderEditor.getState(/* () */0)));
