...
function removeFirst(arr) {
  ...
  return /* tuple */[
          OptionService$WonderEditor.unsafeGet(Caml_option.undefined_to_opt(arr.shift())),
          arr
        ];
}
...
function hasItemByFunc(func, arr) {
  var arr$1 = arr.filter(Curry.__1(func));
...
function excludeWithFunc(excludeArr, isNeedExcludeFunc, arr) {
  return arr.filter((function (value) {
                return !Curry._2(isNeedExcludeFunc, excludeArr, value);
              }));
}
...
function hasExclude(excludeArr, arr) {
  return exclude(excludeArr, arr).length > 0;
}
...
function _fastConcatArrays(firstArr, remainArr) {
  Contract$WonderLog.requireCheck((function (param) {
          return Contract$WonderLog.test(Log$WonderLog.buildAssertMessage("length >= 1", "not"), (function (param) {
                        return Contract$WonderLog.Operators[/* >= */7](remainArr.length, 0);
                      }));
        }), StateEditorService$WonderEditor.getStateIsDebug(/* () */0));
  return ArrayService$WonderCommonlib.reduceOneParam(fastConcat, firstArr, remainArr);
}

function fastMutableConcatArrays(arrayArrays) {
  return _fastConcatArrays(unsafeGetFirst(arrayArrays), arrayArrays.slice(1));
}

function fastImmutableConcatArrays(arrayArrays) {
  return _fastConcatArrays(unsafeGetFirst(arrayArrays).slice(), arrayArrays.slice(1));
}
...
export {
  ...
  fastConcat ,
  _fastConcatArrays ,
  fastMutableConcatArrays ,
  fastImmutableConcatArrays ,
  isEqual ,
  isInclude ,
  _addFailureFunc ,
