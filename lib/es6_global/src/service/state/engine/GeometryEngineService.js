...
function _isAssetGeometry(geometry) {
  return geometry >= 1;
}

function getAllAssetGeometrys(engineState) {
  return GeometryAPI$Wonderjs.getAllGeometrys(engineState).filter(_isAssetGeometry);
}

function _getAllUniqueGeometrys(gameObject, engineState) {
...
function replaceAllGameObjectGeometryToDefaultGeometry(gameObject, targetGeometry, engineState) {
  return ArrayService$WonderCommonlib.reduceOneParam((function (state, gameObject) {
                return GameObjectComponentEngineService$WonderEditor.addGeometryComponent(gameObject, targetGeometry, GameObjectComponentEngineService$WonderEditor.disposeGeometryComponent(gameObject, GameObjectComponentEngineService$WonderEditor.unsafeGetGeometryComponent(gameObject, state), state));
              }), engineState, ArrayService$WonderCommonlib.flatten(_getAllUniqueGeometrys(gameObject, engineState).map((function (geometryIndex) {
                        return GeometryAPI$Wonderjs.unsafeGetGeometryGameObjects(geometryIndex, engineState);
                      }))));
}
...
export {
  ...
  getAllGeometrys ,
  _isAssetGeometry ,
  getAllAssetGeometrys ,
  unsafeGetGeometryGameObjects ,
  _getAllUniqueGeometrys ,
  replaceAllGameObjectGeometryToDefaultGeometry ,
