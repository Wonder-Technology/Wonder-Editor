...
function handleFileByType(fileResult) {
  ...
  return new Promise((function (resolve, _) {
                return _handleSpecificFuncByType(fileResult[/* type_ */1], /* tuple */[
                            ...
                            (function () {
                                var match = TextureUtils$WonderEditor.createTexture(StateLogicService$WonderEditor.getEditEngineState(/* () */0), StateLogicService$WonderEditor.getRunEngineState(/* () */0));
                                var runEngineState = match[2];
                                var editEngineState = match[1];
                                var texture = match[0];
                                return Curry._2(Image$WonderEditor.onload, OptionService$WonderEditor.unsafeGet(fileResult[/* result */2]), (function (loadedImg) {
                                              ...
                                              StateEditorService$WonderEditor.setState(createNodeAndAddToCurrentNodeParent(newIndex, AssetNodeMapEditorService$WonderEditor.setResult(newIndex, TextureUtils$WonderEditor.buildTextureNodeResult(fileResult[/* name */0], texture), editorState)));
                                              return resolve("resolve");
                                            }));
                              })
                          ]);
