...
function removeFolder(dispatch, _) {
  ...
  if (match !== 0) {
    return Log$WonderLog.info("the root treeNode can\'t remove");
  } else {
    StateLogicService$WonderEditor.getAndSetEditorState((function (editorState) {
            return AssetEditorService$WonderEditor.setAsseTree(AssetUtils$WonderEditor.removeSpecificTreeNodeFromAssetTree(AssetUtils$WonderEditor.getTargetTreeNodeId(editorState), AssetEditorService$WonderEditor.unsafeGetAssetTree(editorState)), editorState);
          }));
    Curry._1(dispatch, AppStore$WonderEditor.ReLoad);
    return /* () */0;
  }
...
function buildContent() {
  ...
  if (match) {
    var treeNode = AssetUtils$WonderEditor.getTreeNodeById(AssetUtils$WonderEditor.getTargetTreeNodeId(editorState), ArrayService$WonderEditor.getFirst(match[0]));
    if (treeNode) {
      return showSpecificTreeNodeImage(AssetEditorService$WonderEditor.getFileMap(editorState), treeNode[0][/* imgArray */2]);
    } else {
      return Log$WonderLog.fatal(Log$WonderLog.buildFatalMessage("buildContent", "the treeNode not exist in assetTree", "", "", ""));
    }
  } else {
    return /* array */[];
  }
