...
function handleFolderToFolder(dispatch, targetId, removedId) {
  ...
  if (match !== 0) {
    ...
    var match$1 = AssetUtils$WonderEditor.isTreeNodeRelationError(targetId, Js_option.getExn(AssetUtils$WonderEditor.getSpecificTreeNodeById(removedId, AssetUtils$WonderEditor.getRootTreeNode(editorState))));
    if (match$1 !== 0) {
      return Curry._1(dispatch, AppStore$WonderEditor.ReLoad);
    } else {
      var match$2 = AssetUtils$WonderEditor.removeSpecificTreeNodeFromAssetTree(removedId, assetTree);
      StateEditorService$WonderEditor.setState(AssetEditorService$WonderEditor.setAsseTree(AssetUtils$WonderEditor.insertNewTreeNodeToTargetTreeNode(targetId, match$2[1], match$2[0]), editorState));
      return Curry._1(dispatch, AppStore$WonderEditor.ReLoad);
    }
  }
}
