...
function showSpecificTreeNodeChildren(store, dispatch, currentFile, assetTreeChildren) {
  return assetTreeChildren.map((function (param) {
                var id = param[/* id */0];
                return ReasonReact.element(/* Some */[DomHelper$WonderEditor.getRandomKey(/* () */0)], /* None */0, FolderBox$WonderEditor.make(store, dispatch, "./public/img/11.jpg", id, param[/* name */1], AssetTreeUtils$WonderEditor.getSign(/* () */0), currentFile ? AssetUtils$WonderEditor.isIdEqual(id, currentFile[0]) : /* false */0, /* array */[]));
              }));
}
...
function buildContent(store, dispatch) {
  ...
  if (currentTreeNode) {
    ...
    return showSpecificTreeNodeChildren(store, dispatch, currentFile, treeNode_[/* children */4]).concat(showSpecificTreeNodeImage(store, dispatch, fileMap, currentFile, treeNode_[/* imgArray */2])).concat(showSpecificTreeNodeJson(store, dispatch, fileMap, currentFile, treeNode_[/* jsonArray */3]));
  } else {
