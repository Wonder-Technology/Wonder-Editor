...
function isCurrentNodeIdEqualRootId(assetState) {
  ...
  if (match) {
    return AssetUtils$WonderEditor.isIdEqual(match[0], AssetTreeRootAssetService$WonderEditor.getRootTreeNodeId(assetState));
  } else {
    return true;
  }
}
...
function addFolder(dispatchFunc, _) {
  StateLogicService$WonderEditor.getAndSetAssetState((function (assetState) {
          ...
          return AssetTreeNodeUtils$WonderEditor.createNodeAndAddToCurrentNodeParent(nextIndex, AssetTreeNodeUtils$WonderEditor.addFolderIntoNodeMap(nextIndex, assetState$1));
        }));
...
function remove(dispatchFunc, _) {
  StateLogicService$WonderEditor.getAndSetAssetState((function (assetState) {
          var currentNodeId = CurrentNodeIdAssetService$WonderEditor.unsafeGetCurrentNodeId(assetState);
          var match = AssetUtils$WonderEditor.removeSpecificTreeNode(currentNodeId, AssetTreeRootAssetService$WonderEditor.unsafeGetAssetTreeRoot(assetState));
          var newAssetTreeRoot = match[0];
          var assetState$1 = NodeMapAssetService$WonderEditor.setNodeMap(AssetUtils$WonderEditor.deepRemoveTreeNode(match[1], NodeMapAssetService$WonderEditor.unsafeGetNodeMap(assetState)), assetState);
          ...
          if (match$1) {
            return CurrentNodeIdAssetService$WonderEditor.clearCurrentNodeId(AssetTreeRootAssetService$WonderEditor.setAssetTreeRoot(newAssetTreeRoot, CurrentNodeParentIdAssetService$WonderEditor.clearCurrentNodeParentId(assetState$1)));
          } else {
            return CurrentNodeIdAssetService$WonderEditor.clearCurrentNodeId(AssetTreeRootAssetService$WonderEditor.setAssetTreeRoot(newAssetTreeRoot, assetState$1));
          }
        }));
  Curry._1(dispatchFunc, AppStore$WonderEditor.ReLoad);
  return /* () */0;
