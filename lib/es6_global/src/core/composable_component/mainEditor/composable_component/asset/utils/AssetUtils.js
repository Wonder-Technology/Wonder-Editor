...
function isTreeNodeRelationError(removedId, targetTreeNode) {
  var match = +(removedId === targetTreeNode[/* id */0]);
  if (match !== 0) {
    return /* true */1;
  } else {
    return targetTreeNode[/* children */4].reduce((function (result, child) {
                  if (result !== 0) {
                    return /* true */1;
                  } else {
                    return isTreeNodeRelationError(removedId, child);
                  }
                }), /* false */0);
  }
}
