...
function getRootTreeNodeId(editorState) {
  ...
  if (match) {
    ...
    return StateLogicService$WonderEditor.getEditorState(AssetEditorService$WonderEditor.getIndex);
  }
}

function getTargetTreeNodeId(editorState) {
  var match = AssetEditorService$WonderEditor.getCurrentAssetChildrenNodeParent(editorState);
  if (match) {
    return match[0];
  } else {
...
function getSpecificTreeNodeById(id, node) {
  ...
  if (match !== 0) {
    ...
    return node[/* children */1].reduce((function (resultNode, child) {
                  if (resultNode) {
                    return resultNode;
                  } else {
