...
function isTreeNodeRelationError(targetId, removedId, _) {
  var _iterateTreeNode = function (targetId, removedTreeNode) {
    ...
    if (match !== 0) {
      ...
      return removedTreeNode[/* children */4].reduce((function (result, child) {
                    if (result !== 0) {
                      ...
                      return _iterateTreeNode(targetId, child);
                    }
                  }), /* false */0);
    }
  };
  var editorState = StateEditorService$WonderEditor.getState(/* () */0);
  return _iterateTreeNode(targetId, Js_option.getExn(getSpecificTreeNodeById(removedId, ArrayService$WonderEditor.getFirst(AssetEditorService$WonderEditor.unsafeGetAssetTree(editorState)))));
}
