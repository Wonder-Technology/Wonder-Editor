...
function showSpecificTreeNodeChildren(param, param$1, assetTreeNodeChildrenArr) {
  ...
  return assetTreeNodeChildrenArr.map((function (param) {
                ...
                switch (type_) {
                  ...
                  case 3 : 
                      var textureId = Caml_format.caml_int_of_string(OptionService$WonderEditor.unsafeGet(match[/* result */2]));
                      return ReasonReact.element(/* Some */[DomHelper$WonderEditor.getRandomKey(/* () */0)], /* None */0, FileBox$WonderEditor.make(store, dispatchFunc, /* tuple */[
                                      dragImg,
                                      DomHelper$WonderEditor.getAttribute(StateLogicService$WonderEditor.getEngineStateToGetData((function (param) {
                                                  return BasicSourceTextureEngineService$WonderEditor.unsafeGetSource(textureId, param);
                                                })), "src"),
                                      id,
                                      name,
                                      AssetTreeUtils$WonderEditor.getFlag(/* () */0),
...
function buildContent(param, dragImg, debounceTime) {
  ...
  return showSpecificTreeNodeChildren(/* tuple */[
              ...
              dragImg,
              debounceTime,
              NodeMapAssetService$WonderEditor.unsafeGetNodeMap(assetState),
              CurrentNodeIdAssetService$WonderEditor.getCurrentNodeId(assetState)
            ], OptionService$WonderEditor.unsafeGet(AssetUtils$WonderEditor.getSpecificTreeNodeById(AssetUtils$WonderEditor.getTargetTreeNodeId(assetState), AssetTreeRootAssetService$WonderEditor.unsafeGetAssetTreeRoot(assetState)))[/* children */1]);
}
