...
function buildContent(store, dispatch) {
  ...
  var currentAssetChildrenNodeParent = AssetCurrentAssetChildrenNodeParentEditorService$WonderEditor.getCurrentAssetChildrenNodeParent(editorState);
  var nodeMap = AssetNodeMapEditorService$WonderEditor.unsafeGetNodeMap(editorState);
  if (currentAssetChildrenNodeParent) {
    var currentParentAssetTreeNode = Js_option.getExn(AssetUtils$WonderEditor.getSpecificTreeNodeById(currentAssetChildrenNodeParent[0], AssetTreeRootEditorService$WonderEditor.unsafeGetAssetTreeRoot(editorState)));
    var currentAssetTreeNode = AssetCurrentAssetTreeNodeEditorService$WonderEditor.getCurrentAssetTreeNode(editorState);
    return showSpecificTreeNodeChildren(store, dispatch, nodeMap, currentAssetTreeNode, currentParentAssetTreeNode[/* children */1]);
  } else {
    return Log$WonderLog.fatal(Log$WonderLog.buildFatalMessage("buildContent", "the treeNode:" + (String(currentAssetChildrenNodeParent) + " not exist in assetTreeRoot"), "", "", ""));
  }
}
