...
function isFlag(startFlag) {
  if (startFlag !== undefined) {
    return startFlag === AssetUtils$WonderEditor.getFlag(/* () */0);
  } else {
    return false;
  }
}
...
function isTypeValid(startId, assetState) {
  if (startId !== undefined) {
    return Js_option.isSome(SparseMapService$WonderCommonlib.get(startId, TextureNodeMapAssetService$WonderEditor.getTextureNodeMap(assetState)));
  } else {
    return false;
  }
}
...
function showMapComponent(retainedProps) {
  ...
  if (match !== undefined) {
    return React.createElement("img", {
                src: SparseMapService$WonderCommonlib.unsafeGet(match, ImageBase64MapAssetService$WonderEditor.getImageBase64Map(StateAssetService$WonderEditor.getState(/* () */0)))
              });
  } else {
    return React.createElement("img", {
