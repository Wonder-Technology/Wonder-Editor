  ...
  if (currentSelectSource) {
    ...
    if (currentSelectSource[0]) {
      if (currentNodeId) {
        ...
        return ReasonReact.element(/* Some */[DomHelper$WonderEditor.getRandomKey(/* () */0)], /* None */0, AssetTreeInspector$WonderEditor.make(store, dispatchFunc, nodeId, SparseMapService$WonderCommonlib.unsafeGet(nodeId, NodeMapAssetService$WonderEditor.unsafeGetNodeMap(StateAssetService$WonderEditor.getState(/* () */0))), /* array */[]));
      } else {
        return null;
      }
    } else {
...
function make(store, dispatchFunc, allShowComponentConfig, _) {
  ...
  return /* record */[
          ...
          /* retainedProps : record */[
            ...
            /* currentNodeId */CurrentNodeIdAssetService$WonderEditor.getCurrentNodeId(assetState)
          ],
