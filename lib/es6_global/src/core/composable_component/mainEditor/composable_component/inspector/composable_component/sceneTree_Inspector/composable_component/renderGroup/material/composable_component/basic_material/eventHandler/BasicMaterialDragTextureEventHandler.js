...
function _handleSetMap(gameObject, materialComponent, mapId, engineStateToGetData) {
  ...
  if (match !== undefined) {
    return OperateTextureLogicService$WonderEditor.changeTextureMapAndRereshEngineState(materialComponent, mapId, OperateBasicMaterialLogicService$WonderEditor.setBasicMaterialMapToEngineState);
  } else {
    ...
    return OperateTextureLogicService$WonderEditor.replaceMaterialComponentFromNoMapToHasMap(/* tuple */[
              ...
              ], OperateBasicMaterialLogicService$WonderEditor.setBasicMaterialMapToEngineState);
  }
}

function _handleGeometryAddMap(gameObject, param, mapId, engineStateToGetData) {
  var match = GeometryService$WonderEditor.hasTexCoords(GeometryEngineService$WonderEditor.getGeometryTexCoords(param[0], engineStateToGetData));
  if (match) {
    return _handleSetMap(gameObject, param[1], mapId, engineStateToGetData);
  } else {
...
function handleSelfLogic(param, materialComponent, dragedId) {
  ...
  if (match) {
    _handleGeometryAddMap(gameObject, /* tuple */[
          GameObjectComponentEngineService$WonderEditor.unsafeGetGeometryComponent(gameObject, engineStateToGetData),
          materialComponent
        ], param$1[/* textureIndex */0], engineStateToGetData);
  }
  Curry._1(param[1], [
