...
import * as GameObjectUtils$WonderEditor from "../../../../../../utils/scene/GameObjectUtils.js";
...
function onDrop(param, _, param$1) {
  var currentDragSource = param$1[2];
  var dragedUid = param$1[1];
  var targetUid = param$1[0];
  if (currentDragSource) {
    var match = currentDragSource[0] === SceneTreeUtils$WonderEditor.getFlag(/* () */0);
    if (match) {
      ...
      return Log$WonderLog.warn("can\'t drop to sceneTree");
    }
  } else {
    return Log$WonderLog.warn("can\'t drop to sceneTree");
  }
