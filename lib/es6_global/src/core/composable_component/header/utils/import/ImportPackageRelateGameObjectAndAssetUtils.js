...
function _isSceneImageDataEqual(sceneImageUint8Array, uint8Array) {
  if (uint8Array !== undefined) {
    return Js_primitive.valFromOption(uint8Array).length === GenerateSceneGraphEngineService$WonderEditor.readUint32DataFromUint8Array(sceneImageUint8Array);
  } else {
    return true;
  }
}
...
function _isImageDataEqual(param, image2, texture2, imageUint8ArrayDataMap) {
  ...
  if (match !== undefined) {
    ...
    if (match$1) {
      return _isSceneImageDataEqual(uint8Array2, param[3]);
    } else {
...
function _replaceGameObjectMaterialComponentToMaterialAsset(gameObject, param, isLightMaterialDataEqualFunc, engineState) {
  var match = RelateGameObjectAndMaterialAssetUtils$WonderEditor.getRelatedMaterialDataFromGameObject(gameObject, SparseMapService$WonderCommonlib.createEmpty(/* () */0), param[2], param[0], param[1], isLightMaterialDataEqualFunc, engineState);
...
function _replaceSceneGameObjectGeometryComponentToWDBAssetGeometryComponent(gameObject, defaultGeometryData, wdbAssetGameObjectGeometryDataArr, _, engineState) {
  ...
  if (match !== undefined) {
    ...
    var match$1 = RelateGameObjectAndGeometryAssetUtils$WonderEditor.getTargetGeometryByJudgeDefaultGeometry(sceneGameObjectGeometryData, defaultGeometryData, RelateGameObjectAndGeometryAssetUtils$WonderEditor.isGeometryDataEqualForDefaultGeometry, engineState);
...
function relateWDBAssetGameObjectsAndAssets(allWDBGameObjectsArr, param, imageUint8ArrayDataMap) {
  ...
  var defaultGeometryData = RelateGameObjectAndGeometryAssetUtils$WonderEditor.getDefaultGeometryData(editorState, engineState);
  var engineState$1 = ArrayService$WonderCommonlib.reduceOneParam((function (engineState, gameObject) {
          return RelateGameObjectAndGeometryAssetUtils$WonderEditor.replaceWDBAssetGameObjectGeometryComponentToDefaultGeometryComponent(gameObject, defaultGeometryData, _replaceWDBAssetGameObjectMaterialComponentToMaterialAsset(gameObject, defaultMaterialData, /* tuple */[
                          basicMaterialDataMap,
                          lightMaterialDataMap
                        ], imageUint8ArrayDataMap, engineState));
        }), engineState, allWDBGameObjectsArr);
