...
function generateASB(editorState, engineState) {
  ...
  var asb = new ArrayBuffer(totalByteLength);
  var dataView = DataViewUtils$WonderEditor.create(asb);
  var byteOffset = _writeHeader(jsonByteLength, bufferTotalAlignedByteLength, dataView);
  var emptyEncodedUint8Data = _getEmptyEncodedUint8Data(/* () */0);
  var match$4 = _writeJson(byteOffset, /* tuple */[
        emptyEncodedUint8Data,
        jsonAlignedByteLength,
        jsonUint8Array
      ], dataView);
  _writeBuffer(match$4[0], /* tuple */[
        imageBufferViewArr,
        wdbBufferViewArr
      ], match$1[0], match$1[1], match$4[2].buffer);
  return asb;
}
