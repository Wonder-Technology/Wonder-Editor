...
function isImageDataEqual(param, image2, texture2, imageUint8ArrayDataMap) {
  if (isImageNameEqual(param[0], image2) && Caml_obj.caml_equal(param[1], ImageUtils$WonderEditor.getImageWidth(image2)) && Caml_obj.caml_equal(param[2], ImageUtils$WonderEditor.getImageHeight(image2))) {
    ...
    if (match !== undefined) {
      return Uint8ArrayService$WonderEditor.isUint8ArrayEqual(param[3], Caml_option.some(match[1]));
    } else {
      return true;
    }
  } else {
...
function getRelatedTextureData(gameObject, param, param$1, param$2) {
  ...
  if (match !== undefined) {
    ...
    if (match$1 !== undefined) {
      ...
      var targetTexture = _findTextureAsset(param[1], sourceTexture, param[2], engineState);
      match$2 = /* tuple */[
        targetTexture,
...
function getRelatedTextureDataFromGameObject(gameObject, param, param$1) {
  ...
  if (match) {
    ...
    if (match$1) {
      return getRelatedTextureData(gameObject, /* tuple */[
                  replacedTargetTextureMap,
                  param[1],
                  param[2]
                ], /* tuple */[
...
function replaceToTextureAssetTextureComponent(gameObject, param, param$1) {
  ...
  if (targetTexture !== undefined && setMapFunc !== undefined) {
    ...
    if (match !== undefined) {
      return Curry._3(setMapFunc, Caml_option.valFromOption(targetTexture), match, engineState);
    } else {
      return engineState;
    }
  } else {
