...
function MakeEventHandler(EventItem) {
  ...
  var onMarkRedoUndoByStackLastReturnStore = function (reduxTuple, prepareTuple, dataTuple) {
    var newStore = Curry._3(EventItem[/* onMarkRedoUndoByStackLastReturnStore */4], reduxTuple, prepareTuple, dataTuple);
    return MarkRedoUndoEventHandlerUtils$WonderEditor.markRedoUndoChangeNothing(AllStateData$WonderEditor.getHistoryState(/* () */0), newStore, StateHistoryService$WonderEditor.getStateForHistory(/* () */0));
  };
  return /* module */[
          /* onSelect */onSelect,
          /* onDrop */onDrop,
