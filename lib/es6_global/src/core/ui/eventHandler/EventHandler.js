...
function MakeEventHandler(EventItem) {
  ...
  var pushUndoStackWithCopiedEngineStateForPromise = function (reduxTuple, prepareTuple, dataTuple) {
    var uiState = reduxTuple[0];
    return Curry._3(EventItem[/* setUndoValueToCopiedEngineStateForPromise */2], reduxTuple, prepareTuple, dataTuple).then((function (engineState) {
                  StoreHistoryUtils$WonderEditor.storeHistoryStateWithCopiedEngineState(uiState, /* tuple */[
                        StateEditorService$WonderEditor.getState(/* () */0),
                        engineState
                      ]);
  ...
  return /* module */[
          ...
          /* pushUndoStackWithCopiedEngineStateForPromise */pushUndoStackWithCopiedEngineStateForPromise,
