...
function markRedoUndoChangeNothing(historyState, uiState, stateTuple) {
  ...
  if (match) {
    var match$1 = match[0];
    return _storeMarkRedoUndoState(uiState, stateTuple, AllHistoryService$WonderEditor.storeHistoryState(match$1[0], match$1[1], match$1[2], _removeMarkRedoUndoFirst(historyState)));
  } else {
