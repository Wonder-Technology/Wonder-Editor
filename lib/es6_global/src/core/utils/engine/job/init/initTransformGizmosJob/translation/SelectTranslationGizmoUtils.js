...
function isSelectTranslationAxisGizmo(translationAxisGizmo, ray, engineState, _) {
  return ArrayService$WonderCommonlib.reduceOneParam((function (isSelect, gameObject) {
                if (isSelect) {
                  return isSelect;
                } else {
                  var aabb = AABBShapeUtils$WonderEditor.setFromPoints(GeometryEngineService$WonderEditor.getGeometryVertices(GameObjectComponentEngineService$WonderEditor.unsafeGetGeometryComponent(gameObject, engineState), engineState));
                  var halfExtendsLength = Vector3Service$WonderEditor.length(AABBShapeUtils$WonderEditor.getHalfExtends(aabb));
                  return RayUtils$WonderEditor.isIntersectOBB(AABBShapeUtils$WonderEditor.expandByScalar(0.3 * halfExtendsLength, aabb), TransformEngineService$WonderEditor.getLocalToWorldMatrixTypeArray(GameObjectComponentEngineService$WonderEditor.unsafeGetTransformComponent(gameObject, engineState), engineState), ray);
                }
              }), false, HierarchyGameObjectEngineService$WonderEditor.getAllChildren(translationAxisGizmo, engineState));
}

function _isSelectTranslationPlaneGizmo(translationPlaneGizmo, ray, engineState, _) {
  return isIntersectMesh(translationPlaneGizmo, ray, engineState);
}

function _unsafeGetIntersectPointWithPlane(plane, ray, _) {
  var match = RayUtils$WonderEditor.checkIntersectPlane(plane, ray);
  if (match !== undefined) {
    return match;
  } else {
