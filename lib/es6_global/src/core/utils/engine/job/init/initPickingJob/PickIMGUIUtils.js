...
function findPickedIMGUIGameObject($$event, editorState, engineState) {
  ...
  return ArrayService$WonderEditor.getFirst(EditIMGUIFuncUtils$WonderEditor.getIMGUIGameObjects(SceneEngineService$WonderEditor.getSceneGameObject(engineState), engineState).filter((function (imguiGameObject) {
                    return _isPickIMGUIGameObject(Vector2Service$WonderEditor.convertIntToFloat(locationInView), EditIMGUIFuncUtils$WonderEditor.computePositionAndSize(imguiGameObject, editorState, engineState));
                  })));
}
