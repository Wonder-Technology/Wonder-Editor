...
function _calcCenterAndDistance(targetGameObject, radiusRatio, engineState) {
  ...
  if (allPointsAndLocalToWolrdMatrices.length === 0) {
    ...
    return /* tuple */[
            center,
            AABBShapeUtils$WonderEditor.calcRadiusOfAABB(aabb, center) * radiusRatio
          ];
  }
}

function setCameraFocusTargetGameObject(camera, targetGameObject, radiusRatio, engineState) {
  Contract$WonderLog.requireCheck((function (param) {
          return Contract$WonderLog.test(Log$WonderLog.buildAssertMessage("the editor camera should has arcballCameraController component", "not"), (function (param) {
                        return Contract$WonderLog.assertTrue(GameObjectComponentEngineService$WonderEditor.hasArcballCameraControllerComponent(camera, engineState));
                      }));
        }), StateEditorService$WonderEditor.getStateIsDebug(/* () */0));
  var editorCameraArcballControllerComponent = GameObjectComponentEngineService$WonderEditor.unsafeGetArcballCameraControllerComponent(camera, engineState);
  var match = _calcCenterAndDistance(targetGameObject, radiusRatio, engineState);
...
export {
  ...
  setCameraFocusTargetGameObject ,
