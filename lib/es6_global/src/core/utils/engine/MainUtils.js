...
function handleRunEngineState(runEngineState) {
  ...
  var match = DefaultSceneUtils$WonderEditor.prepareDefaultComponentForRunEngineState(editorState, runEngineState);
  var match$1 = DefaultSceneUtils$WonderEditor.createDefaultSceneForRunEngineState(match[2], match[0], match[1]);
  var editorState$1 = match$1[0];
  StateLogicService$WonderEditor.setRunEngineState(DirectorEngineService$WonderEditor.loopBody(0, DirectorEngineService$WonderEditor.init(GameObjectEngineService$WonderEditor.setGameObjectName("scene", scene, _setRunEnginestateUnsafeGetStateFuncAndSetStateFuncForEvent(match$1[1])))));
  StateEditorService$WonderEditor.setState(AssetIndexEditorService$WonderEditor.setLastDefaultComponentIndex(AssetIndexEditorService$WonderEditor.getIndex(editorState$1), editorState$1));
  return /* () */0;
}
