...
function _getSceneCameras(_, allGameObjects, engineState) {
...
function _getSceneDirectionLights(_, allGameObjects, engineState) {
  return allGameObjects.filter((function (gameObject) {
                return GameObjectComponentEngineService$WonderEditor.hasDirectionLightComponent(gameObject, engineState);
              }));
}

function _getScenePointLights(_, allGameObjects, engineState) {
  return allGameObjects.filter((function (gameObject) {
...
function _getImageMaxWidth() {
  return 30;
}

function _getImageMaxHeight() {
  return 30;
}

function _getMaxDistance() {
  return 500;
}
...
function _drawLight(_, name, sceneLights, imageFunc, editorState, engineState) {
...
function _drawDirectionLight(maxDistance, scene, allGameObjects, imageFunc, editorState, engineState) {
  return _drawLight(/* tuple */[
              maxDistance,
              scene,
              allGameObjects
            ], "directionLight", _getSceneDirectionLights(scene, allGameObjects, engineState), imageFunc, editorState, engineState);
}

function _drawPointLight(maxDistance, scene, allGameObjects, imageFunc, editorState, engineState) {
  return _drawLight(/* tuple */[
              maxDistance,
              scene,
              allGameObjects
            ], "pointLight", _getScenePointLights(scene, allGameObjects, engineState), imageFunc, editorState, engineState);
}

function _drawSceneCamera(_, scene, allGameObjects, imageFunc, editorState, engineState) {
  return ArrayService$WonderCommonlib.reduceOneParam((function (engineState, sceneCameraGameObject) {
              ...
              }), engineState, _getSceneCameras(scene, allGameObjects, engineState));
}
...
function getEngineStateIMGUIFunc() {
  return (function (_, apiJsObj, engineState) {
      ...
      var allGameObjects = HierarchyGameObjectEngineService$WonderEditor.getAllGameObjects(scene, engineState);
      return _drawSceneCamera(500, scene, allGameObjects, imageFunc, editorState, _drawPointLight(500, scene, allGameObjects, imageFunc, editorState, _drawDirectionLight(500, scene, allGameObjects, imageFunc, editorState, engineState)));
    });
}
