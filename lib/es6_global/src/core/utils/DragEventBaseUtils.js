...
function _isTreeNodeRelationValid(targetId, startId, handleRelationError) {
  if (startId) {
    return !StateLogicService$WonderEditor.getStateToGetData(Curry._2(handleRelationError, targetId, startId[0]));
  } else {
    return false;
  }
}

function isTriggerDragEnter(id, handleFlag, handleRelationError) {
  var match = CurrentDragSourceEditorService$WonderEditor.getCurrentDragSource(StateEditorService$WonderEditor.getState(/* () */0));
  if (Curry._1(handleFlag, match[0])) {
...
function isTriggerDragLeave(id, handleFlag, handleRelationError) {
  var match = CurrentDragSourceEditorService$WonderEditor.getCurrentDragSource(StateEditorService$WonderEditor.getState(/* () */0));
  if (Curry._1(handleFlag, match[0])) {
    return _isTreeNodeRelationValid(id, match[1], handleRelationError);
  } else {
...
function isTriggerDragDrop(id, startId, handleFlag, handleRelationError) {
  var match = CurrentDragSourceEditorService$WonderEditor.getCurrentDragSource(StateEditorService$WonderEditor.getState(/* () */0));
  if (Curry._1(handleFlag, match[0])) {
    return _isTreeNodeRelationValid(id, /* Some */[startId], handleRelationError);
  } else {
    return false;
  }
}
