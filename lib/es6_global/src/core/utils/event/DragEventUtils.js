...
function handleDragEnter(id, param, isWidgetFunc, checkNodeRelationFunc, _) {
  var match = DragEventBaseUtils$WonderEditor.isValidForDragEnter(id, isWidgetFunc, checkNodeRelationFunc);
  if (match[0]) {
    return param[0];
  } else {
    return param[1];
...
function handleDrop(id, param, isWidgetFunc, checkNodeRelationFunc, $$event) {
  ...
  var match = DragEventBaseUtils$WonderEditor.isValidForDragDrop(id, startId, isWidgetFunc, checkNodeRelationFunc);
