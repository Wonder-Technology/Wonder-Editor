...
function _loop() {
  var _loopRequest = function () {
    return requestAnimationFrame((function (time) {
                  var engineState = EngineStateFacade$WonderEditor.getEngineState(/* () */0);
                  EngineStateFacade$WonderEditor.setEngineState(MainEditorMainBuss$WonderEditor.loopBody(time, engineState));
                  _loopRequest(time);
                  return /* () */0;
                }));
  };
...
function start() {
  var match = _init(EditorStateFacade$WonderEditor.getEditorState(/* () */0));
  _loop(/* () */0);
  return /* tuple */[
          EditorStateFacade$WonderEditor.setEditorState(match[0]),
          EngineStateFacade$WonderEditor.setEngineState(match[1])
        ];
}
