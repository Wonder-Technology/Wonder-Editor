...
function _isDragedGameObjectBeTargetGameObjectParent(targetGameObject, dragedGameObject, engineState) {
  var _targetTransform = GameObjectLogicSingleService$WonderEditor.getTransformComponent(targetGameObject, engineState);
  var dragedTransform = GameObjectLogicSingleService$WonderEditor.getTransformComponent(dragedGameObject, engineState);
...
function setParent(parentGameObject, childGameObject, param) {
  ...
  return /* tuple */[
          param[0],
          MainEditorTransformOper$WonderEditor.setParent(GameObjectLogicSingleService$WonderEditor.getTransformComponent(parentGameObject, engineState), GameObjectLogicSingleService$WonderEditor.getTransformComponent(childGameObject, engineState), engineState)
        ];
}
...
function setTransformParentKeepOrder(parentGameObject, childGameObject, param) {
  ...
  return /* tuple */[
          param[0],
          MainEditorTransformOper$WonderEditor.setTransformParentKeepOrder(GameObjectLogicSingleService$WonderEditor.getTransformComponent(parentGameObject, engineState), GameObjectLogicSingleService$WonderEditor.getTransformComponent(childGameObject, engineState), engineState)
        ];
}

function _getGameObjectName(gameObject, engineState) {
  var match = GameObjectLogicSingleService$WonderEditor.hasCameraControllerComponent(gameObject, engineState);
  if (match !== 0) {
    return "camera";
  } else {
...
function _buildSceneGraphData(gameObject, engineState) {
  var _buildSceneGraphDataRec = function (gameObject, treeNode, engineState) {
    var match = GameObjectLogicCompositeService$WonderEditor.hasChildren(gameObject, engineState);
    if (match !== 0) {
      return GameObjectLogicCompositeService$WonderEditor.getChildren(gameObject, engineState).reduce((function (treeNode, child) {
                    return /* record */[
...
function buildSceneGraphDataWithNewGameObject(newGameObject, oldSceneGraphData, param) {
  var scene = OperateArrayUtils$WonderEditor.getFirst(oldSceneGraphData);
  return /* array */[/* record */[
            /* name */scene[/* name */0],
            /* uid */scene[/* uid */1],
            /* children */OperateArrayUtils$WonderEditor.push(_buildTreeNode(newGameObject, param[1]), scene[/* children */2].slice())
          ]];
}
...
function getDragedSceneGraphData(targetUid, dragedUid, sceneGraphArrayData) {
  return Contract$WonderLog.ensureCheck((function (dragedSceneGraph) {
                return Contract$WonderLog.test(Log$WonderLog.buildAssertMessage("the draged scene graph data == scene data from engine", "not"), (function () {
                              return Contract$WonderLog.assertTrue(Caml_obj.caml_equal(StateFacade$WonderEditor.getState(getSceneGraphDataFromEngine), dragedSceneGraph));
                            }));
              }), EditorStateDataEdit$WonderEditor.getStateIsDebug(/* () */0), _insertRemovedTreeNodeToTargetTreeNode(targetUid, _removeDragedTreeNodeFromSceneGrahph(dragedUid, sceneGraphArrayData)));
}
