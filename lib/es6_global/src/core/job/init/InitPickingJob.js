...
function _isIntersectSphere(ray, param, param$1) {
  return RayUtils$WonderEditor.isIntersectSphere(SphereShapeUtils$WonderEditor.applyMatrix4(PickingEditorService$WonderEditor.unsafeGetSphereShape(param[2], param$1[0]), param[3]), ray);
}
...
function _findPickedOne($$event, allGameObjectData, param) {
  ...
  return _getTopOne(cameraGameObject, engineState, allGameObjectData.filter((function (data) {
...
function _handlePicking($$event, engineState) {
  ...
  var engineState$1 = pickedGameObject !== undefined ? _handleSceneTreeCurrentNodeAndRedoUndo(Js_primitive.valFromOption(pickedGameObject), engineState) : _handleSceneTreeCurrentNodeAndRedoUndo(undefined, engineState);
