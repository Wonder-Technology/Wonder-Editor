...
function _setWholeGizmoRotation(currentSceneTreeNode, wholeGizmo, currentGizmoType, coordinateSystem, _, engineState) {
  var wholeGizmoTransform = GameObjectComponentEngineService$WonderEditor.unsafeGetTransformComponent(wholeGizmo, engineState);
  if (currentGizmoType >= 2) {
    var currentSceneTreeNodeTransform = GameObjectComponentEngineService$WonderEditor.unsafeGetTransformComponent(currentSceneTreeNode, engineState);
...
function updateTransformJob(_, engineState) {
  ...
  if (match) {
    ...
    return _setWholeGizmoRotation(currentSceneTreeNode, wholeGizmo, currentGizmoType, CoordinateSystemTransformGizmoSceneViewEditorService$WonderEditor.getCoordinateSystem(editorState), editorState, engineState$1);
  } else {
    return engineState;
  }
}
