...
function handleDragDrop(_, param, onDragDropFunc) {
  var state = param[1];
...
function renderLabel(param, label, onDragDropFunc) {
  if (label !== undefined) {
    var state = param[1];
    var send = param[0];
...
function render(label, param, param$1) {
  ...
  return React.createElement("article", {
              className: "inspector-item wonder-float-input"
                  ...
                  send,
                  state
                ], label, param[1]), renderContent(/* tuple */[
                  send,
                  state
                ]));
}

function make($staropt$star, $staropt$star$1, defaultValue, label, onChange, onBlur, _) {
  var canBeZero = $staropt$star !== undefined ? $staropt$star : true;
  var onDragDrop = $staropt$star$1 !== undefined ? $staropt$star$1 : (function () {
        return /* () */0;
      });
