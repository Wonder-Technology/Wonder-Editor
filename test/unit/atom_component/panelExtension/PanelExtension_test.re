              ...
              describe(
                ...
                () => {
                  ...
                  test(
                    ...
                    () => _test(ExtensionTool.getNotFindAtomCaseText(), "_getUniqueAtomAttribute")
                  );
                  test(
                    "if extension add error atom component type, log error message and continue",
                    () =>
                      _test(ExtensionTool.getAttributeTypeErrorCaseText(), "_createArgumentArray")
                  );
                  test(
                ...
                () => {
                  test(
                    ...
                    () =>
                      ...
                      |> toThrowMessageRe([%re {|/appState->mapState->componentsMap is none/img|}])
                  );
                  test(
                    ...
                    () =>
                      expect(
                        () => {
                          let extensionText = ExtensionTool.getExtensionText();
                          let specificExtensionText = ExtensionTool.getExtensionSpecificCaseText();
                          ReactTestRenderer.create(
                            ExtensionTool.buildSpecificExtesion(
                              "App",
                              extensionText,
