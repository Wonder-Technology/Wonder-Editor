...
let _ =
  describe("PanelExtension", () => {
    ...
    describe("test build component", () => {
      test("accord to user json data, build component", () => {
      ...
      describe("test exception", () => {
        describe("test error", () => {
          let _test = (extensionText, expectedMsg) => {
            ...
            ConsoleTool.getMessage(error) |> expect |> toContain(expectedMsg);
          };
          test(
        ...
        describe("test fatal", () => {
          test("if can't set map in the store, fatal", () =>
            expect(() => {
              let extensionText = ExtensionTool.getExtensionText();
              ReactTestRenderer.create(
                ExtensionTool.buildSpecificExtesion(
            ...
            expect(() => {
              ...
              ReactTestRenderer.create(
                ExtensionTool.buildSpecificExtesion(
                  ...
                  extensionText,
                  0,
                  ExtensionTool.buildFakeExtensionAppState(
                    specificExtensionText,
                  ),
                ),
              );
            })
