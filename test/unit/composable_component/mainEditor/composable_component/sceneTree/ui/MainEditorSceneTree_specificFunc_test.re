    ...
    describe("test getDragedSceneGraphData function", () => {
      describe(
        ...
        describe("test has children case", () => {
          describe("has first layer children", () => {
            test("add into first layer parent", () => {
              let dragedSceneGraph =
      ...
      describe("deal with specific case", () =>
        test("test if drageId is can't find in array, should throw error", () =>
          expect(() =>
            getDragedSceneGraphData(1, 5, getSimpleSceneTree())
          )
          |> toThrowMessageRe(
               [%re
                 {|/expect dragedTreeNode should exist, but actual not/img|}
               ],
             )
        )
      );
