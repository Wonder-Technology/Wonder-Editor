...
let _getUniqueAtomAttribute = (atomName: string) =>
  ...
  |> Js.Array.filter((atom: ExistAtomAttributeType.atomAttributeComponent) => atom.name === atomName)
  |> (
    (atomAttributeArr) =>
...
let _findUniquePropArrayByAtomName = (atomName, propArray: array(CustomAtomParseType.props)) =>
  propArray
  |> Js.Array.filter((props: CustomAtomParseType.props) => props.name === atomName)
...
let _createArgumentArray =
    (uiComponentName: string, state: AppStore.appState, prop: CustomAtomParseType.props) =>
  prop
  |> (
    ({name, value, type_}) =>
      switch type_ {
  ...
  CustomAtomParseType.(
    ...
    |> (
      (propArray: Js.Array.t(props)) =>
        ...
        | 0 => None
        | _ =>
          propArray |> ArrayService.unsafeGetFirst |> _createArgumentArray(uiComponentName, state)
        }
    )
  );
...
let _buildComponentWithArgument = (component: CustomAtomParseType.atomComponent, argumentArray) =>
  argumentArray
...
let _parseSystem =
    (uiComponentName: string, state: AppStore.appState, atomComponent: CustomAtomParseType.atomComponent) =>
  atomComponent.name
